...
if((isset($_GET['furl'])) || (isset($_GET['code']))) {
    if (isset($_GET['furl'])) {
        ...
        $query = "SELECT id,code,name,banner,logo,bg_color,header_html,footer_html,meta_title,meta_description,meta_image,sort_settings,open_target,cookie_consent,ga_tracking_id FROM svt_showcases WHERE (friendly_url='$furl' OR code='$furl');";
    }
    if (isset($_GET['code'])) {
        $code = $_GET['code'];
        $query = "SELECT id,code,name,banner,logo,bg_color,header_html,footer_html,meta_title,meta_description,meta_image,sort_settings,open_target,cookie_consent,ga_tracking_id FROM svt_showcases WHERE code='$code';";
    }
    ...
    if ($result) {
        if ($result->num_rows==1) {
            ...
            $query_list = "SELECT v.id,v.date_created,s.type_viewer,s.priority,v.code,v.author,v.name as title,v.description,v.background_image as image,r.panorama_image,r.min_yaw,r.max_yaw,r.haov,r.vaov,r.hfov,GROUP_CONCAT(DISTINCT c.id) as id_category,COUNT(al.id) as total_access
                            ...
                            GROUP BY v.id,v.date_created,s.type_viewer,s.priority,v.code,v.author,v.name,v.description,v.background_image,r.panorama_image,r.min_yaw,r.max_yaw,r.haov,r.vaov,r.hfov;";
...
$font_backend = "";
$cookie_policy = "";
$query = "SELECT language,language_domain,font_provider,font_backend,cookie_policy FROM svt_settings LIMIT 1;";
$result = $mysqli->query($query);
if($result) {
    if($result->num_rows==1) {
        $row=$result->fetch_array(MYSQLI_ASSOC);
...
<div class="showcase noselect">
    ...
    <div class='categories'>
        ...
        <div id="search_div" class="mb-1 d-inline-block">
            <input placeholder="<?php echo _("Search"); ?>..." type="text" class="form-control search_input" style="width:160px">
        </div>
    </div>
    <section>
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '.vt-card', function(e) {
            ...
            if(window.open_target=='new') {
                window.open('../'+type+'/index.php?code='+vt_code,'_blank');
            } else {
