...
$query = "SELECT COUNT(*) as num FROM svt_rooms WHERE virtual_staging=2 AND id IN (SELECT id FROM svt_rooms WHERE id_virtualtour=$id_virtualtour);";
$result = $mysqli->query($query);
if($result) {
    if($result->num_rows==1) {
        $row=$result->fetch_array(MYSQLI_ASSOC);
        if($row['num']>0) $use_live_p=true;
    }
}
...
$query = "SELECT COUNT(*) as num FROM svt_pois WHERE type='object3d' OR embed_type='object3d' AND id_room IN (SELECT id FROM svt_rooms WHERE id_virtualtour=$id_virtualtour);";
$result = $mysqli->query($query);
if($result) {
    if($result->num_rows==1) {
        $row=$result->fetch_array(MYSQLI_ASSOC);
...
$query = "SELECT COUNT(*) as num FROM svt_measures WHERE id_room IN (SELECT id FROM svt_rooms WHERE id_virtualtour=$id_virtualtour);";
$result = $mysqli->query($query);
if($result) {
    if($result->num_rows==1) {
        $row=$result->fetch_array(MYSQLI_ASSOC);
        if($row['num']>0) $use_measure=true;
    }
}
if($preview==1) {
    $flyin=0;
...
?>
    <!DOCTYPE HTML>
    <html lang="<?php echo $lang_code; ?>" style="background-color:<?php echo $loading_background_color; ?>">
    <head>
        ...
        <?php endif; ?>
        <?php if($use_object3d) : ?>
            <script type="module" src="js/model-viewer.min.js?v=3.5.0"></script>
        <?php endif; ?>
        <?php if($meeting!=0) : ?>
        ...
        <?php endif; ?>
            <script src="js/howler.min.js?v=2.2.4"></script>
        <?php } else { ?>
        ...
        <?php if($use_live_session) : ?>
            <script type="text/javascript" src="js/peerjs.min.js?v=1.4.7"></script>
        ...
        <?php endif; ?>
        <?php if($use_object3d) : ?>
            <script type="module" src="js/model-viewer.min.js?v=3.4.0"></script>
        <?php endif; ?>
    ...
    <div id="vt_container">
        ...
        <div id="floatingSocialShare">
            ...
            <div class="a2a_kit a2a_kit_size_32 a2a_default_style">
                <?php if(in_array('copy_link',$array_share_providers_settings)) : ?><a class="a2a_button_copy_link <?php echo (in_array('copy_link',$array_share_providers) ? '' : 'hidden'); ?>"></a><?php endif; ?>
            ...
            <script>
                ...
                function share_config(share_data) {
                    switch(show_share) {
                        case 1:
                            var url_vt =  removeURLParameters(location.href, ['nostat', 'ignore_embedded']);
                            return {
                                url: url_vt,
                            };
                            break;
                        case 2:
                            ...
                            if(window.friendly_url!=='') {
                                if(window.vt_language_force!=='') {
                                    var url_vt = window.base_url+'viewer/'+window.friendly_url+'?room='+current_id_panorama+'&yaw='+current_yaw+'&pitch='+current_pitch+'&hfov='+current_hfov+'&lang='+window.vt_language_force;
                                } else {
                                    var url_vt = window.base_url+'viewer/'+window.friendly_url+'?room='+current_id_panorama+'&yaw='+current_yaw+'&pitch='+current_pitch+'&hfov='+current_hfov;
                                }
                            } else {
                                if(window.vt_language_force!=='') {
                                    var url_vt = window.base_url+'viewer/index.php?code='+window.code+'&room='+current_id_panorama+'&yaw='+current_yaw+'&pitch='+current_pitch+'&hfov='+current_hfov+'&lang='+window.vt_language_force;
                                } else {
                                    var url_vt = window.base_url+'viewer/index.php?code='+window.code+'&room='+current_id_panorama+'&yaw='+current_yaw+'&pitch='+current_pitch+'&hfov='+current_hfov;
                                }
                            }
                            return {
                                url: url_vt,
        ...
        <div id="div_panoramas">
            ...
            <div id="controls_bottom_left">
                ...
                <?php if($ui_style['controls']['facebook']['position']=='left') : ?>
                    <div style="order:<?php echo ($ui_style['controls']['facebook']['order']); ?>;" title="<?php echo _("Facebook Chat"); ?>" class="small-element controls_btn facebook_control tooltip loading_spinner_icon <?php echo ($ui_style['controls']['facebook']['type']=='menu') ? 'hidden' : ''; ?>" onclick="view_facebook_messenger()"><?php echo print_library_icon('facebook','icon'); ?><i style="<?php echo ($ui_style['controls']['facebook']['icon_library']!=0) ? 'display:none':''; ?>" class="<?php echo $ui_style['controls']['facebook']['icon']; ?>"></i></div>
                <?php endif; ?>
            ...
            <div class="language_menu noselect">
                ...
                <div class="dropdown">
                    <?php
                    if ((array_key_exists('en_US', $vt_languages_enabled) && $vt_languages_enabled['en_US'] == 1) && (array_key_exists('en_GB', $vt_languages_enabled) && $vt_languages_enabled['en_GB'] == 1)) {
                        $en_gb = "English (British)";
                        $en_us = "English (American)";
                    } else if (!(array_key_exists('en_US', $vt_languages_enabled) && $vt_languages_enabled['en_US'] == 1) && (array_key_exists('en_GB', $vt_languages_enabled) && $vt_languages_enabled['en_GB'] == 1)) {
                        $en_gb = "English";
                        $en_us = "English";
                    } else if ((array_key_exists('en_US', $vt_languages_enabled) && $vt_languages_enabled['en_US'] == 1) && !(array_key_exists('en_GB', $vt_languages_enabled) && $vt_languages_enabled['en_GB'] == 1)) {
                        $en_gb = "English";
                    ...
                    foreach($languages as $code_l => $name_l) {
                        if(array_key_exists($code_l, $vt_languages_enabled) && $vt_languages_enabled[$code_l] == 1) : ?>
                            <p id="lang_item_<?php echo $code_l; ?>" class="<?php echo ($language==$code_l) ? 'active' : ''; ?>" onclick="change_language_vt('<?php echo $code_l; ?>')">
                                <img src="css/flags_lang/<?php echo $code_l; ?>.png?v=2" />&nbsp;&nbsp;<?php echo $name_l; ?>
                            </p>
                    <?php endif; } ?>
                </div>
            </div>
            ...
            <div class="menu_controls noselect">
                ...
                <div class="dropdown">
                    ...
                    <p style="order:<?php echo ($ui_style['controls']['facebook']['order']); ?>" class="facebook_control loading_spinner_icon <?php echo ($ui_style['controls']['facebook']['type']=='button') ? 'hidden' : ''; ?>" onclick="view_facebook_messenger()"><?php echo print_library_icon('facebook','list'); ?><i style="<?php echo ($ui_style['controls']['facebook']['icon_library']!=0) ? 'display:none':''; ?>" class="<?php echo $ui_style['controls']['facebook']['icon']; ?>"></i>&nbsp;&nbsp;&nbsp;<?php echo _("Facebook Chat"); ?></p>
    ...
    <?php if($cookie_consent) { ?>
        <script type="text/plain" data-category="functionality" data-service="Facebook Chat">
