...
if($result_vt) {
    if ($result_vt->num_rows == 1) {
        ...
        if(!empty($row['list_alt']) && !empty($array_vt_lang['list_alt'])) {
            $row['list_alt']=$array_vt_lang['list_alt'];
        }
...
$query = "SELECT create_landing,create_gallery,create_presentation,enable_live_session,enable_meeting,enable_chat,enable_voice_commands,enable_share,enable_device_orientation,enable_webvr,enable_logo,enable_nadir_logo,enable_song,enable_forms,enable_annotations,enable_rooms_multiple,enable_rooms_protect,enable_info_box,enable_maps,enable_icons_library,enable_password_tour,enable_expiring_dates,enable_auto_rotate,enable_flyin,enable_multires,enable_dollhouse,enable_multilanguage,enable_avatar_video FROM svt_plans as p LEFT JOIN svt_users AS u ON u.id_plan=p.id WHERE u.id = $id_user LIMIT 1;";
                            ...
                            switch($row_p['type']) {
                                case 'gallery':
                                    ...
                                    if($gallery_order=='random') {
                                        $query_g = "SELECT image,title,description FROM svt_poi_gallery WHERE id_poi=$id_poi ORDER BY RAND();";
                                    } else {
