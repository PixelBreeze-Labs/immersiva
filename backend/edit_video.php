...
if($video!==false) {
    ...
    $query = "SELECT image FROM svt_gallery WHERE id_virtualtour=$id_virtualtour ORDER BY priority;";
                        ...
                        <div id="div_upload_slide_image">
                            <form id="frm_si" action="ajax/upload_slide_image.php" method="POST" enctype="multipart/form-data">
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="preview text-center">
                                            <div id="progress_si" class="progress mb-3" style="height: 2.35rem;display: none">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <button id="btn_select_image_gallery" data-toggle="modal" data-target="#modal_gallery_images_vp" class="btn btn-block btn-primary <?php echo ((count($array_gallery_images)==0) ? 'disabled' : '') ?>"><?php echo _("Select image from Gallery"); ?></button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<div id="modal_gallery_images_vp" class="modal" tabindex="-1" role="dialog" data-backdrop="static">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php foreach ($array_gallery_images as $gallery_image) { ?>
                    <img onclick="select_gallery_image('<?php echo $path_base_url; ?>gallery/<?php echo $gallery_image; ?>')" class="float-left mb-2 ml-1 mr-1 image_gallery_slider" data-image="<?php echo $gallery_image; ?>" draggable="false" style="object-fit:cover;width:120px;height:120px;border-radius:5px;" src="<?php echo $path_base_url; ?>gallery/thumb/<?php echo $gallery_image; ?>">
                <?php } ?>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> <?php echo _("Close"); ?></button>
...
<script>
    ...
    window.select_gallery_image = function(file_url) {
        $('#modal_gallery_images_vp').modal('hide');
        $('#div_image_slide_image img').attr('src',file_url);
        $('#div_delete_slide_image').show();
        $('#div_image_slide_image').show();
        $('#div_upload_slide_image').hide();
        $('#txtFile_si').parent().removeClass("error-highlight");
