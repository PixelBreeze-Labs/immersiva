...
$purchase_code_gsv = strip_tags($_POST['purchase_code_gsv']);
...
if (file_exists('../../gsv/index.php')) {
    $query = "UPDATE svt_gsv_settings SET purchase_code=?;";
    if($smt = $mysqli->prepare($query)) {
        $smt->bind_param('s',$purchase_code_gsv);
        $smt->execute();
    }
    $google_maps_api_key = $_POST['google_maps_api_key'];
    $google_oauth2_client_id= $_POST['google_oauth2_client_id'];
    $google_oauth2_client_secret= $_POST['google_oauth2_client_secret'];
    if($google_maps_api_key!='keep_google_maps_api_key') {
        $query = "UPDATE svt_gsv_settings SET google_maps_api_key=?;";
        if($smt = $mysqli->prepare($query)) {
            $smt->bind_param('s',$google_maps_api_key);
            $smt->execute();
        }
    }
    if($google_oauth2_client_id!='keep_google_oauth2_client_id') {
        $query = "UPDATE svt_gsv_settings SET google_oauth2_client_id=?;";
        if($smt = $mysqli->prepare($query)) {
            $smt->bind_param('s',$google_oauth2_client_id);
            $smt->execute();
        }
    }
    if($google_oauth2_client_secret!='keep_google_oauth2_client_secret') {
        $query = "UPDATE svt_gsv_settings SET google_oauth2_client_secret=?;";
        if($smt = $mysqli->prepare($query)) {
            $smt->bind_param('s',$google_oauth2_client_secret);
            $smt->execute();
        }
    }
}
