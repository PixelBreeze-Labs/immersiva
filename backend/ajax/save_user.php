...
$gsv_publish_credits = (int)$_POST['gsv_publish_credits'];
...
$query = "UPDATE svt_users SET username=?,email=?,role=?,super_admin=?,id_plan=?,active=?,language=?,expire_plan_date_manual=?,first_name=?,last_name=?,company=?,tax_id=?,street=?,city=?,province=?,postal_code=?,country=?,tel=?,ai_credits=?,autoenhance_credits=?,gsv_publish_credits=? WHERE id=?;";
if($smt = $mysqli->prepare($query)) {
    $smt->bind_param('sssiiissssssssssssiiii', $username,$email,$role,$super_admin,$id_plan,$active,$language,$expire_plan_date_manual,$first_name,$last_name,$company,$tax_id,$street,$city,$province,$postal_code,$country,$tel,$ai_credits,$autoenhance_credits,$gsv_publish_credits,$id_user);
    $result = $smt->execute();
    if($result) {
